...
2. Drag ```MobFoxSDKCore.embeddedframework``` from the Finder into your project to support Bitcode, or ```MobFoxSDKCoreWithoutBitCode.embeddedframework``` to disable Bitcode. If you provide bitcode (in project settings), all apps and frameworks in the app bundle (all targets in the project) need to include bitcode.
