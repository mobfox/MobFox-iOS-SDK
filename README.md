...
2. Drag ```MobFoxSDKCore.embeddedframework``` from the Finder into your project to support Bitcode (```Enable Bitcode``` should be set to ```YES``` by default), or ```MobFoxSDKCoreWithoutBitCode.embeddedframework``` to disable Bitcode (```Enable Bitcode``` needs to be set to ```NO```). If you provide bitcode (in project settings), all apps and frameworks in the app bundle (all targets in the project) need to include bitcode.
